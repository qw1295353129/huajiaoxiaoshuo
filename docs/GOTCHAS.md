...
## 加主题时，优先覆盖 Tailwind 的颜色变量而不是改类名

新增一套暖色主题（奶油底 + 琥珀橙），要求**默认的中性主题不受影响**。

全应用有 **176 处 `bg-white`**（卡片/面板）和一批 `bg-neutral-50/950`（页面底）。
第一种想法是把它们换成语义类（`bg-surface` 之类），但那样 diff 会有几百行、
还会漏掉动态拼接的类名。

**正确做法**：这些工具类在 Tailwind v4 里最终都解析到 `--color-*` 变量，
所以**只在主题作用域里重定义那几个变量**：

```css
:root[data-theme="warm"] {
  --color-neutral-50: oklch(0.978 0.012 85);   /* 页面底 */
  --color-white:      oklch(0.995 0.006 90);   /* 卡片底 */
  --accent:           oklch(0.68 0.16 55);     /* 强调色 */
}
```

一处生效，全应用跟随，默认主题零改动。

**只覆盖构成"底色"的那几个变量**。文字灰、边框灰保持不动 ——
暖色主题要的是"暖底黑字"，把所有灰都染黄会糊成一片。

**暖色用 `data-theme` 属性而不是再加一个 class**：它与 `dark` 是正交的
（暖阳本身是浅色系），属性写在 CSS 里是 `[data-theme="warm"]`，
一眼能看出是"主题"而不是"明暗模式"。

**切回浅色必须显式清掉属性**，否则会"粘住"（选了浅色还是黄的，刷新也不恢复）。
回归里专门有一条守这个。

## 测试里判断颜色：浏览器不会帮你把 oklch 转成 rgb

两种想当然的办法**都不行**：

- `el.style.color = v; getComputedStyle(el).color` → **原样返回 oklch**；
- Canvas 的 `ctx.fillStyle = v` → 同样不认 oklch，也是原样返回。

结果是我的颜色断言在 oklch 上"看起来通过了"（因为判定函数对无法解析的值默认放行），
实际什么都没验。
